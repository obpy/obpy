...
class TransmissionLayer(asyncore.dispatcher):
    ...
    def handle_read(self):
        ...
        if address not in self._ack_bitfield:
            self._ack_bitfield[address] = collections.deque()
            
        packet_history = list(reversed(self._ack_bitfield[address]))
        for packet_num, packet in enumerate(packet_history):
            pass
    ...
    def writable(self):
        return sum(map(lambda l: len(l), self._queued_packet.iteritems())) >= 0
    
    def _send_packet(address, packet_type_id, optional_data = ''):
        ...
        packed_packet_type_id = str(packet_type_id).zfill(PACKET_TYPE_ID_LENGTH)
